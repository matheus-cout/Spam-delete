...
class GmailSpamManagerApp:
    ...
    def create_widgets(self):
        ...
        ttk.Checkbutton(cred_frame, text="Lembrar credenciais", variable=self.remember_credentials).pack(anchor=tk.W, pady=5)
    ...
    def load_saved_credentials(self):
        """Carrega as credenciais salvas mais recentes, se disponíveis."""
        email, password = cm.get_credentials()
        if email:
            self.email_var.set(email)
            self.password_var.set(password)
            self.status_var.set(f"Credenciais carregadas para: {email}")

    def on_email_selected(self, event):
        """Carrega a senha quando um email é selecionado no dropdown."""
        email = self.email_var.get()
        if email:
            _, password = cm.get_credentials(email)
            if password:
                self.password_var.set(password)
                self.status_var.set(f"Credenciais carregadas para: {email}")
